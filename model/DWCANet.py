...
class DWT_Downsample(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        # Wavelet transform increases channels by 4x (LL, LH, HL, HH)
        self.conv = nn.Conv2d(in_ch * 4, out_ch, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn = nn.BatchNorm2d(out_ch)
        self.relu = nn.ReLU(inplace=True)

    def forward(self, x):
        # 1. Slice operations similar to Haar wavelet (parameter-free, fast)
        x01 = x[:, :, 0::2, :] / 2
        x02 = x[:, :, 1::2, :] / 2
        x1 = x01[:, :, :, 0::2]
        x2 = x02[:, :, :, 0::2]
        ...
        # 2. Calculate four frequency bands
        x_LL = x1 + x2 + x3 + x4  # Low frequency (approx. average pooling)
        x_HL = -x1 - x2 + x3 + x4  # High frequency (horizontal details)
        x_LH = -x1 + x2 - x3 + x4  # High frequency (vertical details)
        x_HH = x1 - x2 - x3 + x4  # High frequency (diagonal details)

        x_cat = torch.cat([x_LL, x_HL, x_LH, x_HH], dim=1)

        return self.relu(self.bn(self.conv(x_cat)))
...
class Res_CBAM_CoordAtt_Block(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        super(Res_CBAM_CoordAtt_Block, self).__init__()
...
class DWCANet(nn.Module):
    def __init__(self, num_classes=1,input_channels=1, block=Res_CBAM_CoordAtt_Block, num_blocks=[2, 2, 2, 2], nb_filter=[16, 32, 64, 128, 256], deep_supervision=True, mode='test'):
        super(DWCANet, self).__init__()
        self.mode = mode
        self.relu = nn.ReLU(inplace = True)
        self.deep_supervision = deep_supervision

        self.pool0 = DWT_Downsample(nb_filter[0], nb_filter[0])  # 16 -> 16
        self.pool1 = DWT_Downsample(nb_filter[1], nb_filter[1])  # 32 -> 32
        self.pool2 = DWT_Downsample(nb_filter[2], nb_filter[2])  # 64 -> 64
        self.pool3 = DWT_Downsample(nb_filter[3], nb_filter[3])  # 128 -> 128
    ...
    def forward(self, input):
        ...
        x4_0 = self.conv4_0(self.pool3(x3_0))# [1, 256, 16, 16]
